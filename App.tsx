...
import { 
  FileText, Download, Trash2, AlertCircle, CheckCircle, Loader2, Settings, Zap, Sparkles, ChevronDown, RefreshCw, Languages, Plus, Search, Link2, Book, Brain, Type, Volume2, VolumeX, SkipBack, SkipForward, LogOut, Eye, EyeOff, Menu, ScrollText, Key, ExternalLink, Github, HelpCircle, AlertTriangle, X, PlusCircle, History, Hourglass, Info, Wand2, FileArchive, ArrowRight, Play, Pause, Square, Sliders, Coffee, Sun, Moon, FileOutput, Save, BookOpen, ToggleLeft, ToggleRight, Wand, UploadCloud, Smartphone, Maximize2, Minimize2, MoreHorizontal, FileSearch, PlayCircle, ShieldCheck, BatteryWarning, Activity
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const playTTS = useCallback((startIndex: number = 0) => {
    ...
    utterance.onerror = (event) => {
        if (event.error !== 'interrupted') setIsTTSPlaying(false);
    };
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-white rounded-[3rem] w-full max-w-xl p-12 shadow-premium space-y-8 animate-in zoom-in-95 duration-300 relative">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={saveApiKey} className="flex-[2] bg-indigo-600 text-white font-bold py-5 rounded-[1.5rem] shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-3"><Save className="w-5 h-5" /> Lưu Cấu Hình</button>
            </div>
          </div>
        </div>
      )}
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar px-6 pt-24 pb-32" onClick={() => setIsReaderUIHidden(!isReaderUIHidden)}>
                  <div className="max-w-3xl mx-auto space-y-4">
                    ...
                    {viewingChapter.translatedContent?.split('\n').filter(p => p.trim()).map((line, idx) => (
                        <p key={idx} ref={activeTTSIndex === idx ? activeLineRef : null} onClick={(e) => { e.stopPropagation(); playTTS(idx); }} className={`p-4 rounded-2xl transition-all border-2 border-transparent hover:bg-black/5 text-lg leading-relaxed cursor-pointer ${activeTTSIndex === idx ? 'bg-indigo-500/10 border-indigo-500/30 font-medium' : 'opacity-90'}`} style={{ fontSize: `${readerSettings.fontSize}px` }}>{line.trim()}</p>
                    ))}
                  </div>
              </div>
